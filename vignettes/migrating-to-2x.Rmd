...
vignette: >
  %\VignetteIndexEntry{Migrating from 1.x to 2.x}
...
| Function | 1.x | 2.x |
|------------------------------------|------------------|------------------|
| `add_atc_classification()` | `unnest = TRUE` | `as_list_column = TRUE` |
| `pivot_medication_data_wide()` | `position_col` optional | `position_col` **required** (no default) |
| `get_single_rxcui()` | had `retry_count` | `retry_count` removed (retries now happen at the batch layer) |

For `add_atc_classification()`, the behavior is also slightly different:
with `as_list_column = TRUE` the output is a list-column rather than
exploded rows. To recover the old row-per-class shape, follow with
`tidyr::unnest_longer()`.
...
package. `add_atc_classification()` now calls the NLM RxClass REST API
directly. No code change is required on your side, but if you previously
installed `rxnorm` only for adRpheno, you can remove it.
