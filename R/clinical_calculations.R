...
#' @param bmi_col String. Existing BMI column to preserve as `bmi_original`
...
#'   Note: `weight_col` is renamed to `weight_lbs`, `height_col` to
#'   `height_inch`, and `bmi_col` to `bmi_original`. New columns added:
#'   `bmi_imperial`, `obesity`.
#' @export
#'
#' @examples
#' \dontrun{
...
calculate_bmi_obesity <- function(dataf, weight_col, height_col, bmi_col,
  ...
  adRutils::validate_args(
    ...
    weight_col      = adRutils::is_string(),
    height_col      = adRutils::is_string(),
    bmi_col         = adRutils::is_string(),
    cutoff          = adRutils::is_number(positive = TRUE),
...
#' @param decimal_places Integer. Decimal places for rounding (default: 2)
...
calculate_bp_metrics <- function(dataf, systolic1, systolic2, diastolic1, diastolic2,
  ...
  adRutils::validate_args(
    ...
    columns         = required_cols,
    numeric_columns = required_cols,
    systolic1       = adRutils::is_string(),
    systolic2       = adRutils::is_string(),
    diastolic1      = adRutils::is_string(),
    diastolic2      = adRutils::is_string(),
    calculate_pp    = adRutils::is_flag(),
    calculate_map   = adRutils::is_flag(),
    decimal_places  = adRutils::is_number(min = 0),
    verbose         = adRutils::is_flag(),
    custom_checks   = list(
      list(condition = calculate_map || calculate_pp,
           message   = "At least one of {.arg calculate_map} or {.arg calculate_pp} must be TRUE")
    )
  )

  if (verbose) {
    extras <- c(if (calculate_map) "MAP", if (calculate_pp) "pulse pressure")
    msg <- if (length(extras) > 0) {
      paste0("Calculating average BP and ", paste(extras, collapse = ", "))
    } else {
  ...
  if (calculate_pp) {
    result_df <- result_df %>%
      dplyr::mutate(
        pulse_pressure = round(.data$avg_systolic_bp - .data$avg_diastolic_bp, decimal_places)
      )
  }
...
#'
#' Calculates eGFR using the CKD-EPI 2021 formula based on serum creatinine,
#' age, and sex.
#'
#' @param dataf A data frame containing the input data
#' @param creatinine_col String. Column name for serum creatinine in mg/dL
#' @param age_col String. Column name for age in years
#' @param sex_col String. Column name for sex. Accepts 0/1, "m"/"f",
#'   "male"/"female" (case-insensitive). 1/female = female; 0/male = male.
#' @param egfr_col String. Output column name (default: `"eGFR"`)
#' @param verbose Logical. If `TRUE`, shows informative messages (default: `TRUE`)
#'
#' @details
#' Uses the CKD-EPI (Chronic Kidney Disease Epidemiology Collaboration) 2021
#' equation:
...
calculate_egfr <- function(dataf, creatinine_col, age_col, sex_col,
  ...
  adRutils::validate_args(
    ...
    creatinine_col  = adRutils::is_string(),
    age_col         = adRutils::is_string(),
    sex_col         = adRutils::is_string(),
    egfr_col        = adRutils::is_string(),
    verbose         = adRutils::is_flag()
  )
  ...
  if (egfr_col %in% names(dataf) && verbose) {
  ...
  # CKD-EPI constants by sex (1 = female, 0 = male)
