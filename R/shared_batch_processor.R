...
.process_batch <- function(items, api_function, batch_size = 200, save_freq = 50,
  ...
  if (length(items_to_process) == 0) {
    cli_alert_info("All {process_type} found in cache")
    return(cache_results[items])
  }

  # Show cache statistics
  stats <- adRutils::get_cache_stats(cache)
  cli_alert_info("Cache contains {stats$entry_count} existing entries")

  # Create batches
  batches <- split(items_to_process, ceiling(seq_along(items_to_process) / batch_size))
  cli_alert_info("Processing {length(items_to_process)} new {process_type} in {length(batches)} batches")

  # Initialize CLI progress bar with static format
  pb_id <- cli::cli_progress_bar(
    format = paste0("Processing ", process_type, " [{cli::pb_bar}] {cli::pb_percent} | ETA: {cli::pb_eta}"),
    total = length(items_to_process),
    clear = FALSE
  )

  # Process each batch (NO processed_count tracking)
...
.process_one_batch <- function(batch, api_function, retry_count, pb_id, ...) {
  ...
  for (item in batch) {
    ...
    # Update progress bar for each item processed
